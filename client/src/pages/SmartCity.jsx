...
import { FiArrowRight, FiHome, FiHeart, FiLayers, FiShoppingBag, FiMap, FiTruck, FiShield, FiZap, FiGlobe } from 'react-icons/fi';
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 ${isDark ? 'bg-[#071426]' : 'bg-slate-50'}`}>
      ...
      <section className="relative h-[60vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 text-center">
          <h1 className={`text-5xl md:text-7xl font-black mb-6 tracking-tighter ${isDark ? 'text-white' : 'text-slate-900'}`}>
            Kafumbu <span className="text-emerald-500">Smart City</span>
          </h1>
          <p className={`text-lg md:text-xl max-w-2xl mx-auto leading-relaxed ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
            Une métropole intelligente de 100 000 logements conçue pour la durabilité, la sécurité et l'épanouissement humain.
          </p>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 lg:px-6 py-20">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          ...
          <div className="space-y-8">
            <div>
              ...
              <p className={`text-lg leading-relaxed mb-8 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                La cité moderne de Kafumbu représente une nouvelle génération de développement urbain durable destinée à accueillir plus de 500 000 habitants dans un environnement sécurisé, connecté et économiquement dynamique.
              </p>
            </div>
          ...
          <div className="relative">
            <div className={`absolute -inset-4 rounded-3xl blur-2xl opacity-20 bg-emerald-500`} />
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8 bg-gradient-to-t from-black/80 to-transparent">
                <div className="flex gap-4 md:gap-8">
                  <div className="text-white">
                    <div className="text-2xl md:text-3xl font-black mb-1">100K</div>
                    <div className="text-[10px] font-bold uppercase tracking-widest opacity-70">Maisons</div>
                  </div>
                  <div className="text-white">
                    <div className="text-2xl md:text-3xl font-black mb-1">500K</div>
                    <div className="text-[10px] font-bold uppercase tracking-widest opacity-70">Résidents</div>
                  </div>
                  <div className="text-white">
                    <div className="text-2xl md:text-3xl font-black mb-1">2.5K</div>
                    <div className="text-[10px] font-bold uppercase tracking-widest opacity-70">Hectares</div>
                  </div>
                </div>
              </div>
            </div>
      ...
      <section className={`py-24 ${isDark ? 'bg-white/5' : 'bg-slate-100'}`}>
        <div className="max-w-7xl mx-auto px-4 lg:px-6">
          <div className="text-center mb-16">
            <h2 className={`text-3xl font-black mb-4 ${isDark ? 'text-white' : 'text-slate-900'}`}>Infrastructures de Nouvelle Génération</h2>
            <div className="w-20 h-1.5 bg-emerald-500 mx-auto rounded-full" />
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6">
            {[
              { title: 'Énergie Durable', desc: 'Alimentation 24/7 via le barrage hydroélectrique de 15 MW.', icon: FiZap },
              { title: 'Sécurité Intégrée', desc: 'Réseau de surveillance intelligent et police de proximité.', icon: FiShield },
              { title: 'Connectivité 5G', desc: 'Fibre optique et couverture haut débit dans toute la cité.', icon: FiGlobe },
            ].map((item, i) => (
      ...
      <section className="max-w-5xl mx-auto px-6 py-24 text-center">
        <h2 className={`text-3xl md:text-4xl font-black mb-8 ${isDark ? 'text-white' : 'text-slate-900'}`}>Prêt à faire partie de l'histoire ?</h2>
        <Link 
          to="/investir" 
