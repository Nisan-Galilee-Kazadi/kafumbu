...
export default function Fundraising() {
  ...
  const progress = selectedCampaign
    ? Math.min((selectedCampaign.raised / selectedCampaign.target) * 100, 100)
    : 0;
      ...
      <div className="flex min-h-screen items-center justify-center bg-linear-to-br from-[#63b32e] to-[#0f70b7]">
        <div className="p-8 text-center text-white">
          ...
          <p className="text-lg opacity-90">
            Votre donation met les compteurs a jour en temps reel.
          </p>
        </div>
      </div>
    );
        ...
        <div className="relative container mx-auto px-4 py-20">
          <div className="mx-auto max-w-4xl text-center">
            <h1 className="mb-6 text-4xl font-black md:text-6xl">
              Investissez dans l'Avenir de Kafumbu (KMC)
            </h1>
            <p className="mb-8 text-xl opacity-90 md:text-2xl">
              Rejoignez notre mission pour construire une ville intelligente et
              durable
            </p>
            <div className="flex flex-wrap justify-center gap-4">
              <div className="rounded-xl bg-white/20 p-4 backdrop-blur-sm">
                <p className="text-3xl font-bold">
                  {formatMoney(statistics.totalRaised)}
                </p>
            ...
            >
              <div className="flex items-center justify-between gap-4">
                <div className="flex-1">
                  <h3
                    className={`mb-2 text-xl font-bold ${isDark ? "text-white" : "text-slate-900"}`}
                  >
                    {campaign.title}
                  </h3>
                  <p
                  ...
                  <div className="flex flex-wrap items-center gap-5 text-sm">
                    <span className="flex items-center gap-2">
                      <FiTarget className="text-[#63b32e]" />{" "}
                      {formatMoney(campaign.target)} objectif
                    </span>
                    <span className="flex items-center gap-2">
                      <FiTrendingUp className="text-[#0f70b7]" />{" "}
                      {((campaign.raised / campaign.target) * 100).toFixed(2)}%
                    </span>
                    <span className="flex items-center gap-2">
                      <FiUsers className="text-purple-500" />{" "}
                      {campaign.contributors.toLocaleString()} contributeurs
                    </span>
                  </div>
                </div>
                <div
                  className={`flex h-16 w-16 items-center justify-center rounded-full ${selectedCampaign?.id === campaign.id ? "bg-[#63b32e] text-white" : "bg-gray-200"}`}
                >
                  {selectedCampaign?.id === campaign.id ? (
        ...
        {selectedCampaign && (
          <div className="mx-auto grid max-w-6xl gap-12 lg:grid-cols-2">
            <div className="space-y-8">
              <div
                className={`rounded-2xl border p-8 ${isDark ? "border-white/10 bg-[#0B1D35]" : "border-slate-200 bg-white shadow-lg"}`}
              >
                <h3
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <div
                      className={`h-4 overflow-hidden rounded-full ${isDark ? "bg-[#08172B]" : "bg-slate-200"}`}
                    >
                      <div
                    ...
                    <div className="mt-2 flex justify-between text-sm">
                      ...
                      <span className="font-bold text-[#63b32e]">
                        {progress.toFixed(2)}%
                      </span>
                    </div>
                  </div>
            ...
            <div className="space-y-8">
              ...
              >
                <h3
                  className={`mb-4 text-xl font-bold ${isDark ? "text-white" : "text-slate-900"}`}
                >
                ...
                <p
                  className={`mb-4 text-sm ${isDark ? "text-slate-400" : "text-slate-600"}`}
                >
                  Aidez-nous a etendre notre portee en partageant cette campagne
                  avec votre reseau.
                </p>
                <button
                  className={`flex w-full items-center justify-center gap-2 rounded-lg border p-3 transition ${isDark ? "border-white/10 bg-[#08172B] hover:bg-white/10" : "border-slate-200 bg-white hover:bg-slate-50"}`}
                >
                  <FiShare2 size={16} />
