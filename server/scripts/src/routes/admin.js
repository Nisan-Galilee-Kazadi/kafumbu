...
const getFirstName = (name) => String(name || "").trim().split(/\s+/)[0] || "visiteur";
...
const generateKafumbuUsername = async (conn, name) => {
  ...
  while (true) {
    const [rows] = await conn.execute("SELECT id FROM users WHERE email = ? LIMIT 1", [candidate]);
...
const sendPasswordResetEmail = async ({ email, name, code }) => {
  ...
  await mailer.sendMail({
    ...
    text: `Bonjour ${name},\n\nVotre code de réinitialisation de mot de passe Kafumbu Smart City est ${code}.\nIl expire dans 1 minute 30.\n\nSi vous n'avez pas demande cette reinitialisation, ignorez ce message.`,
  });
};
...
router.post("/register/request", async (req, res) => {
  ...
  try {
    ...
    res
      ...
      .json({ error: "Failed to send verification code", message: err.message });
  }
});
...
router.post("/register/verify", async (req, res) => {
  ...
  try {
    ...
    if (!cleanEmail || !/^[A-Z0-9]{6}$/.test(code)) {
      return res.status(400).json({ error: "Valid email and 6-character code are required" });
    }
    ...
    if (new Date(row.expires_at).getTime() < Date.now()) {
      await conn.execute("DELETE FROM email_verification_tokens WHERE email = ?", [cleanEmail]);
      await conn.commit();
      conn.release();
      return res.status(410).json({ error: "Verification code expired" });
    }

    if (row.attempts >= 5 || row.token_hash !== hashVerificationCode(cleanEmail, code)) {
      await conn.execute(
    ...
    const [existing] = await conn.execute("SELECT id FROM users WHERE email = ?", [platformEmail]);
    if (existing.length > 0) {
      await conn.rollback();
      conn.release();
      return res.status(409).json({ error: "Cet e-mail est deja utilise." });
    ...
    const [result] = await conn.execute(
      "INSERT INTO users (name, email, password, role, tier, company, phone) VALUES (?, ?, ?, 'visitor', ?, ?, ?)",
      [payload.name, platformEmail, payload.passwordHash, payload.tier, payload.company, payload.phone],
    );
    await conn.execute("DELETE FROM email_verification_tokens WHERE email = ?", [cleanEmail]);
    await conn.commit();
    ...
    res.status(500).json({ error: "Failed to verify registration", message: err.message });
  }
});
...
router.get("/users", authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    const [users] = await conn.execute(
      "SELECT id, name, email, role, tier, company, phone, created_at FROM users ORDER BY created_at DESC",
    );
    conn.release();
    res.json(users);
  } catch (err) {
...
router.post("/users", authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    const [result] = await conn.execute(
      "INSERT INTO users (name, email, password, role, tier, company, phone) VALUES (?, ?, ?, ?, ?, ?, ?)",
      [name, email, passwordHash, role, tier, company, phone],
    );
    conn.release();
...
router.put("/users/:id", authMiddleware, adminOnly, async (req, res) => {
  try {
    const { name, email, role, tier, company, phone } = req.body;
    const conn = await pool.getConnection();
...
router.delete("/users/:id", authMiddleware, adminOnly, async (req, res) => {
  ...
  try {
    ...
    await conn.execute("UPDATE notifications SET created_by = NULL WHERE created_by = ?", [
      req.params.id,
    ]);
    await conn.execute("UPDATE settings SET updated_by = NULL WHERE updated_by = ?", [
      req.params.id,
    ]);
...
router.delete("/users/:id/messages", authMiddleware, adminOnly, async (req, res) => {
  try {
    const conn = await pool.getConnection();
    const [result] = await conn.execute(
      "DELETE FROM messages WHERE sender_id = ? OR receiver_id = ?",
    ...
    await conn.execute("DELETE FROM visitor_notifications WHERE user_id = ?", [req.params.id]);
    conn.release();
    res.json({ message: "User messages deleted", deleted: result.affectedRows });
  } catch (err) {
    res.status(500).json({ error: "Failed to delete user messages", message: err.message });
  }
});

router.delete("/users-cleanup/non-primary", authMiddleware, adminOnly, async (req, res) => {
  let conn;
  try {
    ...
    if (targetIds.length === 0) {
      await conn.commit();
      conn.release();
      return res.json({ message: "No users to clean", deletedUsers: 0, deletedMessages: 0 });
    }
    ...
    res.status(500).json({ error: "Failed to clean users", message: err.message });
  }
});
  ...
  try {
    ...
    const [result] = await conn.execute(
      `INSERT INTO media
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [filename, file_path, file_type, mime_type, file_size, width, height, alt_text, title, description, req.user.id],
    );
    conn.release();
    res.status(201).json({ id: result.insertId, message: "Media created" });
  } catch (err) {
    res.status(500).json({ error: "Failed to create media", message: err.message });
  }
});
...
router.put("/notifications/:id/read", authMiddleware, adminOnly, async (req, res) => {
  try {
    const conn = await pool.getConnection();
    await conn.execute("UPDATE notifications SET status = 'read' WHERE id = ?", [req.params.id]);
    conn.release();
...
router.delete("/notifications/:id", authMiddleware, adminOnly, async (req, res) => {
  try {
    const conn = await pool.getConnection();
    await conn.execute("DELETE FROM notifications WHERE id = ?", [req.params.id]);
    conn.release();
...
const mapMessage = (row) => ({
  ...
  senderName: row.sender_name || (row.sender_role === "admin" ? "Admin Kafumbu" : "Visiteur"),
  receiverName: row.receiver_name || (row.receiver_role === "admin" ? "Admin Kafumbu" : "Visiteur"),
  subject: row.subject,
  content: row.content,
...
router.get("/messages/conversations", authMiddleware, adminOnly, async (_req, res) => {
  try {
    const conn = await pool.getConnection();
    const [rows] = await conn.execute(`
      SELECT
...
router.post("/messages/:userId", authMiddleware, adminOnly, async (req, res) => {
  try {
    const { content, subject = "Message de l'administration" } = req.body;
    if (!content?.trim()) return res.status(400).json({ error: "Message content is required" });

    const conn = await pool.getConnection();
    const [userRows] = await conn.execute("SELECT id, name FROM users WHERE id = ? AND role = 'visitor'", [req.params.userId]);
    ...
    await conn.execute(
      ...
      [req.params.userId, "Nouveau message", content.trim().slice(0, 240), req.user.id],
    );
    conn.release();
...
router.get("/visitor/:userId/messages", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to fetch visitor messages", message: err.message });
  }
});
...
router.post("/visitor/:userId/messages", async (req, res) => {
  try {
    ...
    const conn = await pool.getConnection();
    const [visitorRows] = await conn.execute("SELECT id, name FROM users WHERE id = ? AND role = 'visitor'", [req.params.userId]);
    const [adminRows] = await conn.execute("SELECT id FROM users WHERE role = 'admin' ORDER BY id ASC LIMIT 1");
    ...
    await conn.execute(
      `INSERT INTO notifications (title, message, type, status, created_by)
       VALUES (?, ?, 'info', 'unread', ?)`,
      [`Message de ${visitorRows[0].name}`, content.trim().slice(0, 240), req.params.userId],
    );
    conn.release();

    res.status(201).json({ id: result.insertId, message: "Reply sent" });
  } catch (err) {
    res.status(500).json({ error: "Failed to send visitor message", message: err.message });
  }
});
...
router.put("/visitor/:userId/messages/read", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to mark visitor messages", message: err.message });
  }
});
...
router.get("/visitor/:userId/notifications", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to fetch visitor notifications", message: err.message });
  }
});
...
router.put("/visitor/:userId/notifications/:id/read", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to mark visitor notification", message: err.message });
  }
});
...
router.get("/public/content", async (_req, res) => {
  try {
    const conn = await pool.getConnection();
    const [settingsRows] = await conn.execute("SELECT `key`, `value`, `type` FROM settings");
    const [campaigns] = await conn.execute("SELECT * FROM campaigns WHERE status IN ('active','completed') ORDER BY created_at DESC LIMIT 12");
    const [news] = await conn.execute("SELECT * FROM news WHERE status = 'published' ORDER BY published_at DESC, created_at DESC LIMIT 12");
    const [media] = await conn.execute("SELECT * FROM media WHERE status = 'active' ORDER BY created_at DESC LIMIT 24");
    const [blocks] = await conn.execute("SELECT * FROM site_blocks WHERE status = 'published' ORDER BY sort_order ASC, created_at DESC");
    conn.release();

    const settings = {};
    settingsRows.forEach((row) => {
      settings[row.key] = row.type === "json" ? JSON.parse(row.value || "null") : row.value;
    });
    res.json({ settings, campaigns, news, media, blocks });
  } catch (err) {
    res.status(500).json({ error: "Failed to fetch public content", message: err.message });
  }
});
...
router.get("/public/search", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Public search failed", message: err.message });
  }
});
...
router.post("/password/reset-request", async (req, res) => {
  ...
  try {
    ...
    conn = await pool.getConnection();
    const [existing] = await conn.execute("SELECT id, name, email FROM users WHERE email = ?", [cleanEmail]);
    if (existing.length === 0) {
      conn.release();
    ...
    res
      .status(500)
      .json({ error: "Impossible d'envoyer le code de reinitialisation", message: err.message });
  }
});
...
router.post("/password/reset-verify", async (req, res) => {
  ...
  try {
    ...
    if (!cleanEmail || !/^[A-Z0-9]{6}$/.test(code) || !newPassword) {
      return res.status(400).json({ error: "L'e-mail, le code à 6 caractères et le nouveau mot de passe sont requis." });
    }
    ...
    if (rows.length === 0) {
      await conn.rollback();
      conn.release();
      return res.status(404).json({ error: "Demande de verification introuvable." });
    }
    ...
    await conn.execute("UPDATE users SET password = ? WHERE email = ?", [passwordHash, cleanEmail]);
    await conn.execute("DELETE FROM email_verification_tokens WHERE email = ?", [cleanEmail]);
    await conn.commit();
    conn.release();
    ...
    res.status(500).json({ error: "Impossible de reinitialiser le mot de passe", message: err.message });
  }
});
